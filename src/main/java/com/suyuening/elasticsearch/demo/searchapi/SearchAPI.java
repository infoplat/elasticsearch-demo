...
public class SearchAPI {
	public static void main(String[] args) {
		try (Client client = ESClient.client()) {
			SearchResponse response = client.prepareSearch("movielens", "school") // 指定index
					.setTypes("ratings", "student") // 指定type
					.setSearchType(SearchType.DFS_QUERY_THEN_FETCH).setQuery(QueryBuilders.termQuery("movieId", "367")) // Query
					// .setPostFilter(QueryBuilders.rangeQuery("rating").from(4).to(4.5))
					// // Filter
					.setFrom(0).setSize(60).setExplain(true).execute().actionGet();

			// MatchAll on the whole cluster with all default options
			// SearchResponse response =
			// ESClient.client().prepareSearch().execute().actionGet();
